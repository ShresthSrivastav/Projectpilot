...
class TestMultiWorkspace:
    ...
    def test_create_workspace(self):
        ...
        resp = client.post("/api/workspace", json={"name": "Alice Proj"},
    ...
    def test_create_multiple_workspaces(self):
        ...
        client.post("/api/workspace", json={"name": "Carol Work"},
                    headers=_auth_header(reg["access_token"]))
        client.post("/api/workspace", json={"name": "Carol Side"},
                    headers=_auth_header(reg["access_token"]))
        resp = client.get("/api/workspace", headers=_auth_header(reg["access_token"]))
        assert resp.status_code == 200
        workspaces = resp.json()
    ...
    def test_switch_workspace(self):
        ...
        ws2 = client.post("/api/workspace", json={"name": "Dave Side"},
                          headers=_auth_header(reg["access_token"])).json()
        # Switch to it
        switch_resp = client.post("/api/workspace/switch",
        ...
        current_resp = client.get("/api/workspace/current",
...
class TestInvitations:
    ...
    def test_invite_and_accept(self):
        ...
        switch_resp = client.post("/api/workspace/switch",
                                  json={"workspace_id": bob_ws_id},
                                  headers=_auth_header(bob["access_token"]))
        bob_new_token = switch_resp.json()["access_token"]

        invite_resp = client.post("/api/workspace/current/invite",
        ...
        # Alice accepts
        accept_resp = client.post("/api/workspace/accept",
                                  json={"token": token},
                                  headers=_auth_header(alice["access_token"]))
    ...
    def test_invite_wrong_email_fails(self):
        ...
        switch = client.post("/api/workspace/switch", json={"workspace_id": inviter_ws_id},
                             headers=_auth_header(inviter_reg["access_token"])).json()
        ...
        invite = client.post("/api/workspace/current/invite",
                             ...
                             headers=_auth_header(inviter_token)).json()

        # Bob tries to accept with a different email
        accept = client.post("/api/workspace/accept",
    ...
    def test_double_invite_fails(self):
        ...
        client.post("/api/workspace/current/invite",
                    json={"email": "existing@example.com", "role": "MEMBER"},
                    headers=_auth_header(reg["access_token"]))
        invite2 = client.post("/api/workspace/current/invite",
                              json={"email": "existing@example.com", "role": "MEMBER"},
                              headers=_auth_header(reg["access_token"]))
    ...
    def test_pending_invites(self):
        ...
        client.post("/api/workspace/current/invite",
                    ...
                    headers=_auth_header(reg["access_token"]))
        invites = client.get("/api/workspace/current/invites",
                             headers=_auth_header(reg["access_token"]))
...
class TestMembers:
    ...
    def test_members_list(self):
        ...
        members = client.get("/api/workspace/current/members",
                             headers=_auth_header(reg["access_token"])).json()
    ...
    def test_members_after_invite(self):
        ...
        # Owner invites both
        invite_a = client.post("/api/workspace/current/invite",
                               json={"email": "usera@example.com", "role": "MEMBER"},
                               headers=_auth_header(owner["access_token"])).json()
        invite_b = client.post("/api/workspace/current/invite",
                               json={"email": "userb@example.com", "role": "VIEWER"},
                               headers=_auth_header(owner["access_token"])).json()

        # Both accept
        client.post("/api/workspace/accept", json={"token": invite_a["token"]},
                    headers=_auth_header(user_a["access_token"]))
        client.post("/api/workspace/accept", json={"token": invite_b["token"]},
    ...
    def test_remove_member(self):
        ...
        remove = client.delete(f"/api/workspace/current/members/{member_to_remove['id']}",
                               headers=_auth_header(owner["access_token"]))
        assert remove.status_code == 200

        members_after = client.get("/api/workspace/current/members",
                                   headers=_auth_header(owner["access_token"])).json()
...
class TestActivity:
    ...
    def test_activity_on_create(self):
        ...
        resp = client.get("/api/workspace/current/activity",
                          headers=_auth_header(reg["access_token"]))
        assert resp.status_code == 200
    ...
    def test_activity_on_invite(self):
        ...
        client.post("/api/workspace/current/invite",
                    ...
                    headers=_auth_header(owner["access_token"]))
        activities = client.get("/api/workspace/current/activity",
                                headers=_auth_header(owner["access_token"])).json()
...
class TestWorkspaceSwitcher:
    ...
    def test_workspace_create_and_list(self):
        ...
        client.post("/api/workspace", json={"name": "Extra WS"},
                    headers=_auth_header(reg["access_token"]))
        workspaces = client.get("/api/workspace",
                                headers=_auth_header(reg["access_token"])).json()
        assert len(workspaces) == 2

    def test_workspace_switch_updates_token_ws(self):
        reg = _register("User", "tokentest@example.com")
        ws2 = client.post("/api/workspace", json={"name": "Token Test WS"},
                          headers=_auth_header(reg["access_token"])).json()
        ...
        current = client.get("/api/workspace/current",
                             headers=_auth_header(new_token)).json()
