...
# 2. Jobs persistence
jobs = requests.get(f"{BACKEND}/jobs", timeout=5).json()
...
# 4. GitHub routes
try:
    spec = requests.get(f"{BACKEND}/openapi.json", timeout=15).json()
...
r = requests.post(f"{BACKEND}/github/connect", json={"token": "", "username": "test"}, timeout=5)
check("GH connect reject (no token)", r.status_code == 400)

# 6. GitHub connections list
r = requests.get(f"{BACKEND}/github/connections", timeout=5)
check("GH connections endpoint", r.status_code == 200)

# 7. Frontend
r = requests.get(f"{FRONTEND}/healthz", timeout=5)
check(f"Frontend ({r.status_code})", r.status_code == 200)

# 8. Repo search endpoint
r = requests.get(f"{BACKEND}/github/search?q=test&username=test", timeout=5)
