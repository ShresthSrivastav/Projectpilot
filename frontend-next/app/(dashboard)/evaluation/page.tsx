...
export default function EvaluationPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <PageHeader title="Evaluation" description="Quality metrics, analytics, and performance tracking">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="flex items-center justify-center gap-1 mt-1">
              ...
              <p className="text-xs text-muted-foreground">Avg Autonomy</p>
            </div>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-semibold tabular-nums text-info">{totalRuns}</p>
            <div className="flex items-center justify-center gap-1 mt-1">
            ...
            {trendData.length > 0 ? (
              <>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                  <MetricChart title="Autonomy Score Trend" data={trendData} type="line" dataKey="autonomy" color="var(--color-primary)" height={280} />
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
                  <MetricChart title="Success Rate Trend" data={trendData} type="area" dataKey="success" color="var(--color-success)" height={280} formatY={(v) => `${(v * 100).toFixed(0)}%`} />
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                  <MetricChart title="Cost per Run" data={trendData} type="bar" dataKey="cost" color="var(--color-warning)" height={220} formatY={(v) => `$${v.toFixed(2)}`} />
                </motion.div>
              </>
            ) : (
              <Card><CardContent><EmptyState icon={<TrendingUp className="h-12 w-12 opacity-40" />} title="No trend data" description="Run multiple evaluations to see trends over time" /></CardContent></Card>
            )}
          </div>
        </TabsContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {hLoading ? (
                ...
                <EmptyState icon={<ClipboardCheck className="h-12 w-12 opacity-40" />} title="No evaluations yet" description="Run your first evaluation to track quality metrics" />
              ) : (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <Badge variant={(r.status as string) === "complete" ? "success" : (r.status as string) === "failed" ? "destructive" : "warning"} className="text-[10px]">{r.status as string}</Badge>
                          <span className="text-xs text-muted-foreground">{r.created_at ? formatRelativeTime(r.created_at as string) : ""}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-4 text-sm">
                        <span className="tabular-nums"><span className="text-muted-foreground text-[10px]">Autonomy: </span><span className="font-medium">{(r.autonomy_score as number)?.toFixed(1) ?? "-"}</span></span>
                        <span className="tabular-nums"><span className="text-muted-foreground text-[10px]">Success: </span><span className="font-medium">{(r.success_rate as number)?.toFixed(0) ?? "-"}%</span></span>
                        <span className="tabular-nums"><span className="text-muted-foreground text-[10px]">Cost: </span><span className="font-medium">${(r.cost as number)?.toFixed(2) ?? "-"}</span></span>
                      </div>
                    </motion.div>
                  )
                })
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {leaderboardData.length === 0 ? (
                <EmptyState icon={<Trophy className="h-12 w-12 opacity-40" />} title="No leaderboard data" description="Run evaluations to populate the leaderboard" />
              ) : (
                leaderboardData.slice(0, 10).map((entry, i) => {
                  const e = entry as Record<string, unknown>
                  return (
                    <motion.div key={(e.id as string) ?? i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="flex items-center gap-3 rounded-md border border-border p-3">
                      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-muted text-xs font-bold text-muted-foreground">#{i + 1}</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium">{(e.category as string) ?? (e.id as string)?.substring(0, 8)}</p>
                        <p className="text-xs text-muted-foreground">Score: {(e.score as number)?.toFixed(1) ?? "-"}</p>
                      </div>
                      <div className="text-right"><p className="text-lg font-semibold tabular-nums text-primary">{(e.score as number)?.toFixed(1) ?? "-"}</p></div>
                    </motion.div>
                  )
                })
              )}
              ...
              <CardContent>
                ...
                {regressionList.length > 0 && (
                  <div className="mt-3 space-y-1">
                    {regressionList.slice(0, 5).map((reg, i) => {
                      const r = reg as Record<string, unknown>
                      return (
                        <div key={i} className="flex items-center gap-2 text-xs text-muted-foreground">
                          <AlertTriangle className="h-3 w-3 text-warning shrink-0" />
                          <span className="truncate">{(r.description as string) ?? (r.category as string) ?? "Regression"}</span>
                        </div>
                      )
                    })}
                  </div>
