...
def run(generated_files, job_id, blueprint=None, model="local"):
    ...
    for i, f in enumerate(all_findings, 1):
        report_lines.append(
            f"| {i} | {f['severity']} | {f['file']} | {f['line']} | {f['name']}: {f.get('description', '')} | {f['recommendation']} |"
        )
    ...
    log_to_db(
        job_id,
        "SecurityAgent",
        f"Scan complete. CRITICAL={critical_count} HIGH={high_count} MEDIUM={medium_count} LOW={low_count}",
    )
