...
def _gen(prompt: str, model: str, job_id: str, agent: str = "CodeAgent") -> str:
    result = clean_code_response(
        call_model(prompt, system_prompt=_CODE_SYS, model=model or "local",
...
def run(
    ...
    log_to_db(job_id, "CodeAgent",
              f"Submitting {len(parallel_tasks)} files (max_workers={max_workers}, local={is_local})…")

    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        future_to_path = {
            pool.submit(fn): path
            for path, fn in parallel_tasks.items()
        }
            ...
            for path in still_failed:
                if path in parallel_tasks:
                    ...
                    try:
                        ...
                        elapsed = int((time.monotonic() - t0) * 1000)
                        log_to_db(job_id, "CodeAgent", f"{path} serial ({attempt_model}) failed ({elapsed}ms): {exc}", "ERROR")
