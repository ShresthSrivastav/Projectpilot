...
def collect_repo_metrics():
    metrics = {"source_files": 0, "source_loc": 0, "test_files": 0, "test_loc": 0, "total_size_kb": 0}
...
def generate_report(output_path):
    ...
    if vulnerabilities:
        ...
        for v in vulnerabilities:
            ...
            report.append(
                f"| `{v['package']}` | {v['version']} | [{v['id']}](https://github.com/advisories/{v['id']}) | {desc} |"
            )
    else:
    ...
    if outdated_packages:
        ...
        for p in outdated_packages:
            report.append(
                f"| `{p.get('name')}` | {p.get('version')} | {p.get('latest_version')} | {p.get('latest_file_type')} |"
            )
    else:
    ...
    if security_issues:
        ...
        for s in security_issues:
            report.append(
                f"| `{s['file']}:{s['line']}` | {s['issue_text']} | **{s['severity']}** | {s['confidence']} |"
            )
    else:
