        ...
        if job.get("conclusion") == "failure":
            ...
            for step in job.get("steps", []):
                if step.get("conclusion") == "failure":
                    failed_steps.append(
                        {"name": step.get("name"), "number": step.get("number"), "conclusion": step.get("conclusion")}
                    )
            failed_jobs.append(
                {
                    "id": job.get("id"),
                        ...
                        if job_name_sanitized in file_name or str(failed_job["id"]) in file_name:
                            try:
                                with z.open(file_name) as f:
                                    log_lines = [
                                        line.decode("utf-8", errors="replace").rstrip() for line in f.readlines()
                                    ]
