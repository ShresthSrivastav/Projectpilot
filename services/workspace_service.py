...
def invite_to_workspace(db: Session, workspace_id: str, email: str, role: WorkspaceRole, invited_by: str) -> dict:
    ...
    return {"id": invite.id, "email": invite.email, "role": invite.role.value, "token": token, "expires_at": invite.expires_at.isoformat()}
...
def get_pending_invites(db: Session, workspace_id: str, requester_id: str) -> list[dict]:
    ...
    return [{"id": i.id, "email": i.email, "role": i.role.value, "created_at": i.created_at.isoformat() if i.created_at else ""} for i in invites]
